...
import { getModifierNameInPlatform, uint8ArrayToArrayBuffer } from 'utils';
...
class PDFPageLabelSettingsForRange {
    ...
    addNumberingStyleSetting() {
        this.addSetting()
            .setName(t('modal.numbering-style', 'Numbering Style'))
    ...
    addStartSetting() {
        this.addSetting()
            .setName(t('modal.start-counting-from', 'Start counting from'))
            .addText((text) => {
                    ...
                    button
                        ...
                        .onClick(async () => {
                            if (this.pageLabels && this.doc) {
                                ...
                                await this.app.vault.modifyBinary(this.file, uint8ArrayToArrayBuffer(await this.doc.save()));
                            } else {
