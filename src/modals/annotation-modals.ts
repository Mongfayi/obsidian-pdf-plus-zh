...
export class PDFAnnotationEditModal extends PDFAnnotationModal {
    ...
    addOpacitySetting() {
        if (this.oldValues.opacity || this.allowNoValue.opacity) {
            new Setting(this.contentEl)
                .setName(t('modal.opacity', 'Opacity'))
                .addSlider((slider) => {
                    slider
                        .setLimits(0, 1, 0.01)
    ...
    addBorderWidthSetting() {
        if (this.oldValues.borderWidth || this.allowNoValue.borderWidth) {
            new Setting(this.contentEl)
                .setName(t('modal.draw-border', 'Draw border'))
                .addToggle((toggle) => {
    ...
    addContentsSetting() {
        if (this.oldValues.contents || this.allowNoValue.contents) {
            new Setting(this.contentEl)
                .setName(t('modal.comment', 'Comment'))
                .then((setting) => {
    ...
    addButtons() {
        new Setting(this.buttonContainerEl)
            .addButton((button) => {
                button
                    .setButtonText(t('modal.save', 'Save'))
                    .setCta()
                    .onClick(() => {
                        this.onSaveButtonClick();
                    });
...
export class PDFAnnotationDeleteModal extends PDFAnnotationModal {
    onOpen() {
        ...
        this.contentEl.createEl('p', { text: t('modal.are-you-sure-delete-annotation', 'Are you sure you want to delete this annotation\?') });
        if (!this.plugin.settings.warnEveryAnnotationDelete) {
            this.contentEl.createEl('p', { cls: 'mod-warning', text: t('modal.links-pointing-to-annotation', 'There are one or more links pointing to this annotation.') });
        }
