...
export class PDFPageDeleteModal extends PDFPlusModal {
    ...
    onOpen() {
        ...
        this.contentEl.createEl('p', { text: t('modal.are-you-sure-delete-page', 'Are you sure you want to delete this page\?') });
        if (!this.plugin.settings.warnEveryAnnotationDelete) {
            this.contentEl.createEl('p', { cls: 'mod-warning', text: t('modal.links-pointing-to-page', 'There are one or more links pointing to this page.') });
        }

        new Setting(this.contentEl)
            .addButton((button) => {
                button
                    .setButtonText(t('modal.delete', 'Delete'))
                    .setWarning()
                    .onClick(() => {
                        this.#resolve(true);
                        this.close();
...
export class PDFComposerModal extends PDFPlusModal {
    ...
    onOpen() {
        ...
        if (this.askPageLabelUpdateMethod) {
            new Setting(this.contentEl)
                .setName(t('modal.update-page-labels', 'Update the page labels\?'))
        ...
        if (this.askInPlace) {
            new Setting(this.contentEl)
                .setName(t('modal.remove-pages-from-original', 'Remove pages from original file\?'))
                .addToggle((toggle) => {
                    toggle
                        .setValue(inPlace)
                        .onChange((value) => {
        ...
        new Setting(this.contentEl)
            .addButton((button) => {
                button
                    .setButtonText(t('modal.proceed', 'Proceed'))
                    .setCta()
                    .onClick(() => {
...
export class PDFCreateModal extends PDFPlusModal {
    ...
    onOpen() {
        ...
        this.addSetting()
            .setName(t('modal.page-size', 'Page size'))
            .addDropdown((dropdown) => {
        ...
        this.addSetting()
            .setName(t('modal.orientation', 'Orientation'))
            .addDropdown((dropdown) => {
                dropdown
        ...
        this.addSetting()
            .addButton((button) => {
                button
                    .setButtonText(t('modal.create', 'Create'))
                    .setCta()
                    .then((button) => {
                        setTimeout(() => button.buttonEl.focus());
                    })
