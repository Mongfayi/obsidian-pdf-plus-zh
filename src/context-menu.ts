...
export const onThumbnailContextMenu = (plugin: PDFPlus, child: PDFViewerChild, evt: MouseEvent): void => {
    ...
    if (node && node.instanceOf(HTMLElement) && node.hasClass('thumbnail') && node.dataset.pageNumber !== undefined) {
        ...
        const title = ('' + pageNumber === pageLabel)
            ? t('menu.copy-link-to-page-number', 'Copy link to page {number}', { number: pageNumber })
            : t('menu.copy-link-to-page-number-label', 'Copy link to page {label} ({number}/{count})', { label: pageLabel, number: pageNumber, count: pageCount });
        const menu = new Menu()
            .addItem((item) => {
                item.setTitle(title)
                    .setIcon('lucide-copy')
        ...
        if (lib.isEditable(child)) {
            menu.addItem((item) => {
                item.setTitle(t('menu.insert-page-before', 'Insert page before this page'))
                    .setIcon('lucide-plus')
                    .onClick(() => {
                        const file = child.file;
                        if (!file) {
                            new Notice(`${plugin.manifest.name}: ${t('notice.failed-insert-page', 'Failed to insert the page.')}`);
                            return;
                        }
                ...
                .addItem((item) => {
                    item.setTitle(t('menu.insert-page-after', 'Insert page after this page'))
                        .setIcon('lucide-plus')
                        .onClick(() => {
                            const file = child.file;
                            if (!file) {
                ...
                .addItem((item) => {
                    item.setTitle(t('menu.delete-page', 'Delete page'))
                        .setIcon('lucide-trash')
                        .onClick(() => {
                            const file = child.file;
                            if (!file) {
                                new Notice(`${plugin.manifest.name}: ${t('notice.failed-delete-page', 'Failed to delete the page.')}`);
                                return;
                            }
                ...
                .addItem((item) => {
                    item.setTitle(t('menu.extract-page-to-new-file', 'Extract page to new file'))
                        .setIcon('lucide-file-output')
                        .onClick(() => {
                            const file = child.file;
                            if (!file) {
                                new Notice(`${plugin.manifest.name}: ${t('notice.failed-extract-page', 'Failed to extract the page.')}`);
                                return;
                            }
                ...
                .addItem((item) => {
                    item.setTitle(t('menu.divide-document-at-this-page', 'Divide document at this page'))
                        .setIcon('lucide-split-square-vertical')
                        .onClick(() => {
                            const file = child.file;
                            if (!file) {
                                new Notice(`${plugin.manifest.name}: ${t('notice.failed-divide-document', 'Failed to divide the document.')}`);
                                return;
                            }
                ...
                .addSeparator()
                .addItem((item) => {
                    item.setTitle(t('menu.customize', 'Customize...'))
...
export const onOutlineItemContextMenu = (plugin: PDFPlus, child: PDFViewerChild, file: TFile, item: PDFOutlineTreeNode, evt: MouseEvent) => {
    ...
    const title = itemTitle
        ? t('menu.copy-link-to-item', `Copy link to "${itemTitle.length <= 40 ? itemTitle : itemTitle.slice(0, 39).trim() + '\u2026'}"`, { title: itemTitle })
        : t('menu.copy-link-to-section', 'Copy link to section');
    ...
    if (lib.isEditable(child)) {
        menu.addItem((menuItem) => {
            menuItem
                .setTitle(t('menu.add-subitem', 'Add subitem'))
                .setIcon('lucide-plus')
                .onClick(() => {
                    new PDFOutlineTitleModal(plugin, t('modal.add-subitem-title', 'Add subitem to outline'))
                        .ask()
                        .then(async ({ title }) => {
                            ...
                            new Notice(`${plugin.manifest.name}: ${t('notice.failed-add-subitem', 'Failed to add the subitem.')}`);
                            });
                });
        })
            .addItem((menuItem) => {
                menuItem
                    .setTitle(t('menu.rename', 'Rename\.\.\.'))
                    .setIcon('lucide-pencil')
                    .onClick(() => {
                        new PDFOutlineTitleModal(plugin, t('modal.rename-outline-title', 'Rename outline item'))
            ...
            .addItem((menuItem) => {
                menuItem
                    .setTitle(t('menu.move-item-to', 'Move item to\.\.\.'))
                    ...
                    .onClick(async () => {
                        ...
                        if (!itemToMove) {
                            new Notice(`${plugin.manifest.name}: ${t('notice.failed-load-pdf', 'Failed to load the PDF document.')}`);
                            return;
                        }
                        ...
                        new PDFOutlineMoveModal(outlines, itemToMove)
                            ...
                            .then(async (destItem) => {
                                ...
                                await app.vault.modifyBinary(file, uint8ArrayToArrayBuffer(buffer));
                            });
                    });
            })
            .addItem((menuItem) => {
                menuItem
                    .setTitle(t('menu.delete', 'Delete'))
                    .setIcon('lucide-trash')
                    .onClick(async () => {
            ...
            .addItem((menuItem) => {
                menuItem
                    .setTitle(t('menu.extract-to-new-file', 'Extract to new file'))
                    .setIcon('lucide-file-output')
                    .onClick(async () => {
                        ...
                        if (!found) {
                            new Notice(`${plugin.manifest.name}: ${t('notice.failed-process-outline', 'Failed to process the outline item.')}`);
                            return;
                        }
                        ...
                        if (pageNumber === null || nextPageNumber === null) {
                            new Notice(`${plugin.manifest.name}: ${t('notice.failed-fetch-page-numbers', 'Failed to fetch page numbers from the outline item.')}`);
                            return;
                        }

                        if (pageNumber > nextPageNumber) {
                            new Notice(`${plugin.manifest.name}: ${t('notice.invalid-page-numbers', 'The page numbers are invalid: the beginning of this section is page {pageNumber}, whereas the next section starts at page {nextPageNumber}.', { pageNumber, nextPageNumber })}`);
                            return;
                        }
                        ...
                        new PDFComposerModal(
                            ...
                            .then((keepLabels, inPlace) => {
                                lib.composer.extractPages(file, { from: pageNumber, to: nextPageNumber! - 1 }, dstPath, false, keepLabels, inPlace)
                                    .then(async (file) => {
                                        if (!file) {
                                            new Notice(`${plugin.manifest.name}: ${t('notice.failed-extract-section', 'Failed to extract section from PDF.')}`);
                                            return;
                                        }
...
export const onOutlineContextMenu = (plugin: PDFPlus, child: PDFViewerChild, file: TFile, evt: MouseEvent) => {
    ...
    if (lib.isEditable(child)) {
        new Menu()
            .addItem((menuItem) => {
                menuItem
                    .setTitle(t('menu.add-top-level-item', 'Add top-level item'))
                    .setIcon('lucide-plus')
                    .onClick(() => {
                        new PDFOutlineTitleModal(plugin, t('modal.add-item-title', 'Add item to outline'))
                            .ask()
                            .then(async ({ title }) => {
                                const view = lib.getPDFViewFromChild(child);
                                if (view) {
                                ...
                                new Notice(`${plugin.manifest.name}: ${t('notice.failed-add-item', 'Failed to add the item.')}`);
                            });
                    });
            })
    ...
    async addItems(evt?: MouseEvent) {
        ...
        if (Platform.isMacOS && Platform.isDesktopApp && this.win.electron && selectedText && isVisible('action')) {
            this.addItem((item) => {
                return item
                    .setSection('action')
                    .setTitle(t('menu.look-up', `Look up "${selectedText.length <= 25 ? selectedText : selectedText.slice(0, 24).trim() + '\u2026'}"`, { text: selectedText }))
            ...
            if (selectedText && selection && child.palette) {
                if (isVisible('selection')) {
                    PDFPlusProductMenuComponent
                        ...
                        .setSection('selection', t('menu.copy-link-to-selection-section', 'Copy link to selection'), 'lucide-copy')
                ...
                if (lib.isEditable(child) && isVisible('write-file')) {
                    PDFPlusProductMenuComponent
                        .create(this, child.palette)
                        .setSection('write-file', t('menu.add-style-to-file', 'Add {style} to file', { style: plugin.settings.selectionBacklinkVisualizeStyle }), 'lucide-edit')
        ...
        await (async () => {
            if (annot) {
                ...
                if (child.palette && isVisible('annotation')) {
                    PDFPlusProductMenuComponent
                        .create(this, child.palette)
                        .setSection('annotation', t('menu.copy-link-to-annotation-section', 'Copy link to annotation'), 'lucide-copy')
                ...
                if (lib.isEditable(child) && isVisible('modify-annotation')) {
                    if (plugin.settings.enableAnnotationContentEdit && PDFAnnotationEditModal.isSubtypeSupported(annot.data.subtype)) {
                        ...
                        this.addItem((item) => {
                            return item
                                ...
                                .setTitle(t('menu.edit-annotation', 'Edit annotation'))
                                .setIcon('lucide-pencil')
                                .onClick(() => {
                                    if (child.file) {
                    ...
                    if (plugin.settings.enableAnnotationDeletion) {
                        this.addItem((item) => {
                            return item
                                .setSection('modify-annotation')
                                .setTitle(t('menu.delete-annotation', 'Delete annotation'))
                                .setIcon('lucide-trash')
                                .onClick(() => {
                                    if (child.file) {
                    ...
                    if ('dest' in annot.data && typeof annot.data.dest === 'string' && doc && child.file) {
                        ...
                        this.addItem((item) => {
                            item.setSection('link')
                                .setTitle(t('menu.copy-pdf-link', 'Copy PDF link'))
                                .setIcon('lucide-copy')
                                .onClick(async () => {
                        ...
                        if (plugin.lib.isCitationId(destId)) {
                            this.addItem((item) => {
                                item.setSection('link')
                                    .setTitle(t('menu.search-on-google-scholar', 'Search on Google Scholar'))
                                    .setIcon('lucide-search')
                                    .onClick(() => {
                                        const url = this.child.bib?.getGoogleScholarSearchUrlFromDest(destId);

                                        if (typeof url !== 'string') {
                                            new Notice(`${plugin.manifest.name}: ${t('notice.failed-find-bibliography', 'Failed to find bibliographic information.')}`);
                                            return;
                                        }
        ...
        if (selectedText && selection
            ...
            if ('destArray' in plugin.lastCopiedDestInfo) {
                ...
                this.addItem((item) => {
                    return item
                        .setSection('link')
                        .setTitle(t('menu.paste-copied-pdf-link-to-selection', 'Paste copied PDF link to selection'))
                ...
                this.addItem((item) => {
                    return item
                        .setSection('link')
                        .setTitle(t('menu.paste-copied-link-to-selection', 'Paste copied link to selection'))
                        .setIcon('lucide-clipboard-paste')
                        .onClick(() => {
        ...
        if (selectedText && isVisible('text')) {
            this.addItem((item) => {
                return item
                    .setSection('text')
                    .setTitle(t('menu.copy-selected-text', 'Copy selected text'))
                    .setIcon('lucide-copy')
                    .onClick(() => {
                        // How does the electron version differ?
        ...
        if (annotatedText && isVisible('text')) {
            this.addItem((item) => {
                return item
                    .setSection('text')
                    .setTitle(t('menu.copy-annotated-text', 'Copy annotated text'))
                    .setIcon('lucide-copy')
                    .onClick(() => {
                        // How does the electron version differ?
        ...
        if (selectedText && selection && isVisible('search')) {
            this.addItem((item) => {
                item.setSection('search')
                    .setTitle(t('menu.copy-link-to-search', 'Copy link to search'))
                    .setIcon('lucide-search')
                    .onClick(() => {
        ...
        if (lib.speech.isEnabled() && selectedText && isVisible('speech')) {
            this.addItem((item) => {
                item.setSection('speech')
                    .setTitle(t('menu.read-aloud-selected-text', 'Read aloud selected text'))
        ...
        if (!this.items.length && isVisible('page')) {
            this.addItem((item) => {
                item.setSection('page')
                    .setTitle(t('menu.copy-link-to-page-2', 'Copy link to page'))
                    .setIcon('lucide-copy')
                    .onClick((evt) => {
                        const link = child.getMarkdownLink(`#page=${pageNumber}`, child.getPageLinkAlias(pageNumber));
        ...
        if (this.items.length && isVisible('settings')) {
            this.addItem((item) => {
                item.setSection('settings')
                    .setIcon('lucide-settings')
                    .setTitle(t('menu.customize-menu', 'Customize menu\.\.\.'))
                    .onClick(() => {
...
export class PDFPlusProductMenuComponent extends PDFPlusComponent {
    ...
    private addColorItems(menu: Menu) {
        ...
        for (let i = this.showNoColorButton ? -1 : 0; i < colorNames.length; i++) {
            menu.addItem((item) => {
                item.setTitle(i >= 0 ? colorNames[i] : t('menu.dont-specify-color', 'Don\'t specify color'))
                    .onClick((evt) => {
...
export const onBacklinkVisualizerContextMenu = (evt: MouseEvent, visualizer: PDFViewerBacklinkVisualizer, cache: PDFBacklinkCache) => {
    ...
    if (oldColor) {
        menu.addItem((item) => {
            item.setSection('color')
                .setTitle(t('menu.unset-color', 'Unset color'))
    ...
    for (const colorName of Object.keys(settings.colors)) {
        if (colorName.toLowerCase() !== oldColorName?.toLowerCase()) {
            menu.addItem((item) => {
                item.setSection('color')
                    .setTitle(t('menu.change-color-to', 'Change color to "{colorName}"', { colorName }))
                    .setIcon('lucide-palette')
                    .onClick(() => {
    ...
    if (cache.page && cache.FitR) {
        ...
        menu.addItem((item) => {
            item.setSection('image')
                .setTitle(t('menu.copy-as-image', 'Copy as image'))
