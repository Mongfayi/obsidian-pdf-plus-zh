    ...
    constructor(vim: VimBindings) {
        ...
        this.dom = this.vim.viewer.containerEl.createDiv('pdf-plus-vim-command', (el) => {
            ...
            this.inputEl = el.createEl('input', { cls: 'pdf-plus-vim-command-input' }, (inputEl) => {
                inputEl.placeholder = t('placeholder.type-command-or-page', 'type a command or page number...');
    ...
    async executeCommand(cmd: string, options: Partial<{ error: ErrorReportMethod[], history: boolean }> = { error: ['notice', 'console.error'], history: true }) {
        ...
        if (cmd.startsWith('!')) {
            if (!Platform.isDesktopApp) {
                this.reportError(`${this.plugin.manifest.name} (Vim mode): ${t('notice.shell-command-not-supported-on-mobile', 'Shell command is not supported on mobile')}`, options.error!);
                return;
            }
        ...
        if (excmd) {
            if (excmd.minNargs && args.length < excmd.minNargs) {
                this.reportError(`${this.plugin.manifest.name} (Vim mode): ${t('notice.ex-command-args-expected', 'Expected {minArgs} or more arguments for command "{id}" but got {actualArgs}', { minArgs: excmd.minNargs, id: excmd.id, actualArgs: args.length })}`, options.error!);
                return;
            }
            return excmd.func(...args);
        }

        this.reportError(`${this.plugin.manifest.name} (Vim mode): ${t('notice.unknown-command', 'Unknown command "{cmd}"', { cmd })}`, options.error!);
    }
    ...
    submitCommand() {
        ...
        if (cmd) {
            ...
            try {
                ...
                new Notice(`${this.plugin.manifest.name} (Vim mode): ${t('notice.ex-command-error', 'Error occurred while executing the command: {err}', { err: String(err) })}`);
                console.error(err);
            }
        }
        this.vim.enterNormalMode();
