...
import { range, encodeLinktext, uint8ArrayToArrayBuffer } from 'utils';
...
export class PDFFileOperator extends PDFPlusLibSubmodule {
    ...
    async write(path: string, pdfDoc: PDFDocument, existOk: boolean): Promise<TFile | null> {
        ...
        if (file instanceof TFile) {
            ...
            await this.app.vault.modifyBinary(file, uint8ArrayToArrayBuffer(buffer));
            return file;
        } else if (file === null) {
            ...
            return await this.app.vault.createBinary(path, uint8ArrayToArrayBuffer(buffer));
        }
