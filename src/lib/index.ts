...
import { cropCanvas, encodeLinktext, getDirectPDFObj, isVersionNewerThan, parsePDFSubpath, removeExtension, rotateCanvas, toSingleLine, isTargetNode, uint8ArrayToArrayBuffer } from 'utils';
...
export class PDFPlusLib {
    ...
    async getPdfLibDocument(activeOnly: boolean = false) {
        ...
        if (doc) {
            return await this.loadPdfLibDocumentFromArrayBuffer(uint8ArrayToArrayBuffer(await doc.getData()));
        }
    }
    ...
    async getPdfLibPage(activeOnly: boolean = false) {
        ...
        const doc = await this.loadPdfLibDocumentFromArrayBuffer(uint8ArrayToArrayBuffer(await pdfViewer.pdfDocument.getData()));
        if (doc) {
