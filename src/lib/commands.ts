...
import { getObsidianDebugInfo, getStyleSettings, parsePDFSubpath, uint8ArrayToArrayBuffer } from 'utils';
...
export class PDFPlusCommands extends PDFPlusLibSubmodule {
        ...
        const commandArray: Command[] = [
            {
                ...
                name: t('command.copy-link-to-selection', 'Copy link to selection or annotation'),
                checkCallback: (checking) => this.copyLink(checking, false)
            }, {
                id: 'rectangular-selection',
                name: t('command.rectangular-selection', 'Start rectangular selection'),
            ...
            },
            {
                ...
                name: t('command.toggle-adapt-to-theme', 'Toggle "adapt to theme"'),
                checkCallback: (checking) => this.toggleAdaptToTheme(checking)
            }, {
                id: 'go-to-page',
                name: t('command.go-to-page', 'Go to page'),
                checkCallback: (checking) => this.focusAndSelectPageNumberEl(checking)
            }, {
                id: 'copy-format-menu',
                name: t('command.copy-format-menu', 'Show copy format menu'),
                checkCallback: (checking) => this.showCopyFormatMenu(checking)
            }, {
                id: 'display-text-format-menu',
                name: t('command.display-text-format-menu', 'Show display text format menu'),
                checkCallback: (checking) => this.showDisplayTextFormatMenu(checking)
            }, {
                id: 'enable-pdf-edit',
                name: t('command.enable-pdf-edit', 'Enable PDF edit'),
                checkCallback: (checking) => this.setWriteFile(checking, true)
            }, {
                id: 'disable-pdf-edit',
                name: t('command.disable-pdf-edit', 'Disable PDF edit'),
                checkCallback: (checking) => this.setWriteFile(checking, false)
            }, {
                id: 'toggle-auto-focus',
                name: t('command.toggle-auto-focus', 'Toggle auto-focus'),
                callback: () => this.toggleAutoFocus()
            }, {
                id: 'toggle-auto-paste',
                name: t('command.toggle-auto-paste', 'Toggle auto-paste'),
                callback: () => this.toggleAutoPaste()
            }, {
                id: 'toggle-auto-copy',
                name: t('command.toggle-auto-copy', 'Toggle auto-copy'),
                callback: () => this.plugin.autoCopyMode.toggle()
            }, {
                id: 'add-page',
                name: t('command.add-page', 'Add new page at the end'),
                checkCallback: (checking) => this.addPage(checking)
            }, {
                id: 'insert-page-before',
                name: t('command.insert-page-before', 'Insert page before this page'),
                checkCallback: (checking) => this.insertPage(checking, true)
            },
            {
                id: 'insert-page-after',
                name: t('command.insert-page-after', 'Insert page after this page'),
                checkCallback: (checking) => this.insertPage(checking, false)
            }, {
                id: 'delete-page',
                name: t('command.delete-page', 'Delete this page'),
                checkCallback: (checking) => this.deletePage(checking)
            }, {
                id: 'extract-this-page',
                name: t('command.extract-this-page', 'Extract this page to a new file'),
                checkCallback: (checking) => this.extractThisPage(checking)
            }, {
                id: 'divide',
                name: t('command.divide', 'Divide this PDF into two files at this page'),
                checkCallback: (checking) => this.dividePDF(checking)
            }, {
                id: 'edit-page-labels',
                name: t('command.edit-page-labels', 'Edit page labels'),
                checkCallback: (checking) => this.editPageLabels(checking)
            }, {
                id: 'copy-outline-as-list',
                name: t('command.copy-outline-as-list', 'Copy PDF outline as markdown list'),
                checkCallback: (checking) => this.copyOutline(checking, 'list')
            }, {
                id: 'copy-outline-as-headings',
                name: t('command.copy-outline-as-headings', 'Copy PDF outline as markdown headings'),
                checkCallback: (checking) => this.copyOutline(checking, 'heading')
            }, {
                id: 'add-outline-item',
                name: t('command.add-outline-item', 'Add to outline \(bookmark\)'),
                checkCallback: (checking) => this.addOutlineItem(checking)
            }, {
                id: 'create-new-note',
                name: t('command.create-new-note', 'Create new note for auto-focus or auto-paste'),
                callback: () => this.createNewNote()
            }, {
                id: 'copy-debug-info',
                name: t('command.copy-debug-info', 'Copy debug info'),
                callback: () => this.copyDebugInfo()
            }, {
                id: 'load-debug-info',
                name: t('command.load-debug-info', 'Load debug info'),
                checkCallback: (checking) => this.loadDebugInfo(checking)
            }, {
                id: 'create-pdf',
                name: t('command.create-pdf', 'Create new PDF'),
                callback: () => this.createPDF()
            }, {
                id: 'import',
                name: t('command.import', 'Import this PDF into vault'),
                checkCallback: (checking) => this.importExternalFileIntoVault(checking)
            }, {
                id: 'open-external',
                name: t('command.open-external', 'Open this PDF in the original location'),
                checkCallback: (checking) => this.openExternalSource(checking)
            }, {
                id: 'create-dummy',
                name: t('command.create-dummy', 'Create dummy file for external PDF'),
                callback: () => this.createDummyForExternalPDF()
            }, {
                id: 'restore-default',
                name: t('command.restore-default', 'Restore default settings'),
                callback: () => (new RestoreDefaultModal(this.plugin)).open()
            }, {
                id: 'open-dataview-inline-fields-modal',
                name: t('command.open-dataview-inline-fields-modal', 'Check Dataview inline fields'),
    ...
    createPDF() {
        ...
        new PDFCreateModal(this.plugin)
            .askOptions()
            .then(async (doc) => {
                const file = await this.app.vault.createBinary(path, uint8ArrayToArrayBuffer(await doc.save()));
        ...
        if (!checking) {
            new PDFOutlineTitleModal(this.plugin, 'Add to outline')
                ...
                .then(async ({ title }) => {
                    ...
                    await this.app.vault.modifyBinary(file, uint8ArrayToArrayBuffer(await doc.save()));
                });
        }
