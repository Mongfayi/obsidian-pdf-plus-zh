...
import { formatAnnotationID, getBorderRadius, hexToRgb, uint8ArrayToArrayBuffer } from 'utils';
...
export class PdfLibIO extends PDFPlusLibSubmodule implements IPdfIo {
    ...
    async process<T>(file: TFile, fn: (pdfDoc: PDFDocument) => T) {
        ...
        await this.app.vault.modifyBinary(file, uint8ArrayToArrayBuffer(await pdfDoc.save()));
        return ret;
    }
